...
$kelas = base64_decode($_GET['telkom']);
$date = date('d F Y');

$selectKelas = $confg->query("SELECT * FROM tbl_absensi_siswa WHERE kelas = '$kelas' AND tanggal = '$date'");
         ...
         <div class="container flex justify-end items-center mx-auto">
                ...
                <div class="relative overflow-x-auto shadow-md rounded-lg bg-slate-800 text-gray-400 pt-7">
                    <div class="bg-opacity-40 px-4 pb-5">
                            <h1 class="text-xl font-semibold"><b>ABSENSI KELAS</b></h1>
                            <h1 class="">Data Absensi Berdasarkan Kelas, Terbaru Hari ini <?= date('d F Y') ?></h1>
                    </div>
                    <div class="md:flex gap-5 font-medium px-4 pb-5">
                            ...
                            <button class="bg-red-600 p-2 focus:outline-none font-semibold text-white flex items-center justify-center gap-2 rounded-md " onclick="window.location.href= '../components/jadwal_mengajar'"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                ...
                ?>
                    ...
                    >
                        ...
                        <tbody>
                        <?php
                        while($row = mysqli_fetch_array($selectKelas)){
                        ?>
