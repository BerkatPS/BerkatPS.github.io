...
$id = abs($_GET['id']);
$editDataAbsenGuru = $confg->query("SELECT * FROM tbl_absensi_guru WHERE id = $id ");

if(isset($_POST['submit'])){
    $tanggal = htmlspecialchars(htmlentities(mysqli_real_escape_string($confg,$_POST['tanggal'])));
    $nuptk_guru = htmlspecialchars(htmlentities(mysqli_real_escape_string($confg,$_POST['nuptk_guru'])));
    $waktu_kehadiran = htmlspecialchars(htmlentities(mysqli_real_escape_string($confg,$_POST['waktu_kehadiran'])));
    $nama_guru = htmlspecialchars(htmlentities(mysqli_real_escape_string($confg,$_POST['nama_guru'])));
    $status_guru = htmlspecialchars(htmlentities(strtoupper(mysqli_real_escape_string($confg,$_POST['status_guru']))));
    $keterangan = htmlspecialchars(htmlentities(strtoupper(mysqli_real_escape_string($confg,$_POST['keterangan']))));

    if($tanggal != '' OR $nuptk_guru != '' OR $waktu_kehadiran != '' OR $status_guru != '' OR $keterangan != ''){

        $updateDataGuru = mysqli_query($confg,"UPDATE tbl_absensi_guru SET tanggal = '$tanggal', waktu= '$waktu_kehadiran' , nuptk_guru = '$nuptk_guru' , nama = '$nama_guru' , status = '$status_guru' , keterangan = '$keterangan' WHERE id = $id");

        if($updateDataGuru == TRUE){
            header('Location: RekapAbsensi-guru-edit?id='.$id.'&act=successUpdate');
        }else{
            header('Location: RekapAbsensi-guru-edit?id='.$id.'&act=failUpdate');
        }

    }else{
        header('Location: RekapAbsensi-guru-edit?id='.$id.'&act=empty');
    }
}
...
<head>
    ...
    <title>APP KESISWAAN - EDIT REKAP ABSENSI GURU</title>
</head>
...
<div class="md:flex md:flex-row">
        ...
        <div class="bg-slate-800 w-72 text-purple-600 font-mono focus:outline-none z-20 px-6 py-9 absolute inset-y-0 left-0 transform -translate-x-full transition duration-500 ease-in-out lg:relative lg:translate-x-0" id="sidebar">
            ...
            <div class="w-full h-[4rem] m-6 p-4 rounded-lg bg-slate-800 text-zinc-300">
                ...
                <div class="py-3">
                    <?php
                        while($row = mysqli_fetch_array($editDataAbsenGuru)){
                    ?>
                    <form action="" class="bg-slate-800 grid grid-cols-1 p-5 py-6 gap-y-12 gap-x-5 mi-h-full relative" method="POST">
                        <h2 class="font-bold text-2xl sm:text-xl"><button class="px-3 py-1 bg-indigo-500 hover:bg-slate-900" onclick="window.location.href='RekapAbsensi-guru'"><</button>&nbspFORM EDIT ABSENSI GURU</h2>
                        <?php 
                    if(isset($_GET['act'])){
                    ...
                    ?>
                        ...
                        </svg><span class='font-bold '>&nbspSUCCESS UPDATE ABSENSI GURU&nbspAnda Akan Di Arahkan Ke Table Absensi Guru!! </span><span class="flex items-center" id='waktu'>&nbsp3</span>
                        ...
                        if(waktu < 0) {
                            ...
                            window.location = 'RekapAbsensi-guru';
                        }else{
                    ...
                    <?php
                        ...
                        ?>  
                            ...
                            <span class='font-bold'>&nbspGAGAL UPDATE ABSENSI GURU&nbsp</span> DATA TIDAK BOLEH KOSONG!! 
                            </div>
                        <?php
                        }else{
                    ?>  
                        ...
                        <span class='font-bold'>&nbspGAGAL HAPUS DATA&nbsp</span> Anda GAGAL menghapus Data ABSENSI GURU!! 
                        </div>
                    <?php
                    }
                }
                ?>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="nuptk_guru" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan Password Anda" value="<?= $row['nuptk_guru'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">Waktu Kehadiran</label>
                            <input type="text" name="waktu_kehadiran" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan NIS Anda" value="<?= $row['waktu'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">NAMA GURU</label>
                            <input type="text" name="nama_guru" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan Kelas Anda" value="<?= $row['nama'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">STATUS GURU</label>
                            <input type="text" name="status_guru" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan Kelas Anda" value="<?= $row['status'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">KETERANGAN</label>
                            <input type="text" name="keterangan" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan Kelas Anda" value="<?= $row['keterangan'] ?>">
                        </div>
                        <div class=""></div>
