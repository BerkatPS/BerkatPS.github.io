...
if(isset($_POST['simpan'])){
    $title = strtoupper(htmlspecialchars(htmlentities($_POST['title'])));
    $news = strtoupper(htmlspecialchars(htmlentities($_POST['news'])));
    $date = date('d-F-Y H:i:s');
    $date_create = date('d F Y H:i:s');
    if($title != '' OR $news != ''){
            $insertData = mysqli_query($confg,"INSERT INTO news(title,news,tanggal) VALUES('".$title."','".$news."','".$date."')");
            if($insertData == TRUE){
                header('Location: add-news?act=successAdd');
                $insertHistoryPengumumanSiswa = $confg->query("INSERT INTO history_siswa(msgnotif,username,action,information,date_create,status) VALUES('Ada Pengumuman Baru Dari : ','$_SESSION[admin]','PENGUMUMAN','$news','$date_create',0) ");
                $insertHistoryGuru = $confg->query("INSERT INTO history_guru(msgnotif,username,action,information,date_create,status) VALUES('Ada Pengumuman Baru Dari : ','$_SESSION[admin]','PENGUMUMAN','$news','$date_create',0)");
            }else{
                header('Location: add-news?act=failAdd');
            }
    }else{
        header('Location: add-news?act=emtpy');
    }
}
...
<head>
    ...
    <title>APP KESISWAAN - TAMBAH NEWS / PENGUMUMAN</title>
</head>
                    ...
                    <div class="relative">
                        ...
                        <!-- Dropdown Menu -->
                            ...
                            x-transition:enter="transition ease-out duration-200" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95">
                                ...
                                <li>
                                    <a href="../pages/tambaGuru" class="flex items-center hover:text-indigo-500 gap-2 transition duration-200 p-1">
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                    class="group-hover:bg-slate-200 fill-current" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#636e72" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle></svg>
                                    Tambah Data guru</a>
                                </li>
            ...
            <div class="w-full h-[4rem] m-6 p-4 rounded-lg bg-slate-800 text-zinc-300">
                ...
                <div class="py-3">
                    ...
                    ?>
                        ...
                        </svg><span class='font-bold'>&nbspSUCCESS MELAKUKAN TAMBAH NEWS / PENGUMUMAN &nbspAnda Akan Diarahkan ke DASHBOARD&nbsp</span>
                    ...
                    <?php
                        ...
                        <span class='font-bold'>&nbspGAGAL MELAKUKAN TAMBAH NEWS / PENGUMUMAN , DATA TIDAK BOLEH KOSONG</span> 
                        </div>
                    <?php
                       ...
                       <span class='font-bold'>&nbspGAGAL MELAKUKAN TAMBAH NEWS / PENGUMUMAN , DATA NEWS / PENGUMUMAN SUDAH ADA SEBELUMNYA!!</span> 
                       </div>
                   <?php
                   }else{
                         ?>  
                        ...
                        <span class='font-bold'>&nbspGAGAL MELAKUKAN TAMBAH NEWS / PENGUMUMAN , SILAHKAN PERIKSA DATA ANDA KEMBALI</span> 
                        </div>
                    <?php
                    }
                }
                    ?>
                        <h2 class="font-bold text-2xl sm:text-xl"><button class="px-4 py-1 bg-indigo-500 hover:bg-slate-900" onclick="window.location.href= '../'"><</button>&nbspFORM TAMBAH PENGUMUMAN / NEWS</h2>
                        <div class="relative">
                            <label for="">TITLE / JUDUL</label>                            
                            <textarea name="title" id="title" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500"" placeholder="Masukkan Title anda"></textarea>
                        </div>
                        <div class="py-2"></div>
                        <div class="relative">
                            <label for="">NEWS / PENGUMUMAN</label>                            
                            <textarea name="news" id="news" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500"" placeholder="Masukkan News / Pengumuman anda"></textarea>
                        </div>
                        <div class=""></div>
