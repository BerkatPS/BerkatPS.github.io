...
$getNewsId = $confg->query("SELECT * FROM news WHERE id = $_GET[NewsId]");

if(isset($_POST['simpan'])){
    $title = htmlspecialchars(htmlentities(strtoupper($_POST['title'])));
    $news = htmlspecialchars(htmlentities(strtoupper($_POST['news'])));
    if($title != '' OR $news != ''){
            $updateData = mysqli_query($confg,"UPDATE news SET title = '$title', news = '$news' WHERE id = $_GET[NewsId]");
            if($updateData == TRUE){
                header('Location: news-edit?NewsId='.$_GET['NewsId'].'&act=successUpdate');
            }else{
                header('Location: news-edit?NewsId='.$_GET['NewsId'].'&act=failAdd');
            }
    }else{
        header('Location: news-edit?NewsId='.$_GET['NewsId'].'&act=emtpy');
    }
}
?>
...
<head>
    ...
    <title>APP KESISWAAN - EDIT NEWS / PENGUMUMAN</title>
</head>
            ...
            <div class="w-full h-[4rem] m-6 p-4 rounded-lg bg-slate-800 text-zinc-300">
                ...
                <div class="py-3">
                    ...
                    ?>
                        ...
                        </svg><span class='font-bold'>&nbspSUCCESS MELAKUKAN UPDATE NEWS / PENGUMUMAN &nbspAnda Akan Diarahkan ke DASHBOARD&nbsp</span>
                    ...
                    <?php
                        ...
                        <span class='font-bold'>&nbspGAGAL MELAKUKAN UPDATE NEWS / PENGUMUMAN , DATA TIDAK BOLEH KOSONG</span> 
                        </div>
                    <?php
                    }else{
                         ?>  
                    ...
                    ?>
                        <?php
                            while($row = mysqli_fetch_array($getNewsId)){
                        ?>
                        <h2 class="font-bold text-2xl sm:text-xl"><button class="px-4 py-1 bg-indigo-500 hover:bg-slate-900" onclick="window.location.href= '../'"><</button>&nbspFORM EDIT PENGUMUMAN / NEWS</h2>
                        <div class="relative">
                            <label for="">TITLE / JUDUL</label>                            
                            <input name="title" id="title" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan Title anda" value="<?= $row['title'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">NEWS / PENGUMUMAN</label>                            
                            <input name="news" id="news" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left py-20 text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500"" placeholder="Masukkan News / Pengumuman anda" value="<?= $row['news'] ?>">
                        </div>
                        <div class=""></div>
