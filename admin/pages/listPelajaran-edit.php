...
$id = $_GET['id'];
$editDataAbsenSiswa = $confg->query("SELECT * FROM tbl_daftar_pelajaran WHERE id = $id ");

if(isset($_POST['submit'])){
    $list_pelajaran = htmlspecialchars(htmlentities(mysqli_real_escape_string($confg,$_POST['list_pelajaran'])));

    if($list_pelajaran != ''){
        $updateListPelajaran = mysqli_query($confg,"UPDATE tbl_daftar_pelajaran SET list_pelajaran = '$list_pelajaran' WHERE id = $id");

        if($updateListPelajaran == TRUE){
            header('Location: listPelajaran-edit?id='.$id.'&act=successUpdate');
        }else{
            header('Location: listPelajaran-edit?id='.$id.'&act=failUpdate');
        }
    }else{
        header('Location: listPelajaran-edit?id='.$id.'&act=empty');
    }
}
?>
            ...
            <div class="w-full h-[4rem] m-6 p-4 rounded-lg bg-slate-800 text-zinc-300">
                ...
                <div class="py-3">
                    ...
                    ?>
                        ...
                        </svg><span class='font-bold'>&nbspSUCCESS MELAKUKAN UPDATE LIST PELAJARAN &nbspAnda Akan Diarahkan ke LIST PELAJARAN&nbsp</span>
                    ...
                    ?>
                        <div class="relative">
                            <label for="">ID</label>
                            <input type="text" name="tanggal" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan Nama Anda"value="<?= $row['id'] ?>"readonly="readonly">
                        </div>
                        <div class="relative">
                            <label for="">LIST PELAJARAN</label>
                            <input type="text" name="list_pelajaran" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan Nama Anda"value="<?= $row['list_pelajaran'] ?>">
                        </div>
                        <div class=""></div>
