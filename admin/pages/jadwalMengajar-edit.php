...
$id = $_GET['idMengajar'];
$editDataMengajar = $confg->query("SELECT * FROM tbl_pelajaran WHERE id = $id ");

if(isset($_POST['simpan'])){
    $mapel = htmlspecialchars(stripslashes(htmlentities($_POST['mapel'])));
    $guru_pengajar = htmlspecialchars(stripslashes(htmlentities($_POST['guru_pengajar'])));
    $kelas_mengajar = htmlspecialchars(stripslashes(htmlentities($_POST['kelas_mengajar'])));
    $link_virtual = htmlspecialchars(stripslashes(htmlentities($_POST['link_virtual'])));
    $jam_mulai = htmlspecialchars(stripslashes(htmlentities($_POST['jam_mulai'])));
    $jam_akhir = htmlspecialchars(stripslashes(htmlentities($_POST['jam_akhir'])));
    $status_pelajaran = htmlspecialchars(stripslashes(htmlentities(trim(strtoupper($_POST['status_pelajaran'])))));

    if($mapel != '' OR $guru_pengajar != '' OR $kelas_mengajar != '' OR $link_virtual != '' OR $jam_mulai != '' OR $jam_akhir != '' OR $status_pelajaran != ''){
        $updateJadwalMengajar = $confg->query("UPDATE tbl_pelajaran SET mata_pelajaran='$mapel', pengajar='$guru_pengajar',kelas_mengajar='$kelas_mengajar',link_virtual='$link_virtual',jam_mulai='$jam_mulai' , jam_akhir='$jam_akhir' , status='$status_pelajaran' WHERE id = $id");
        if($updateJadwalMengajar == TRUE){
            header('Location: jadwalMengajar-edit?idMengajar='.$id.'&act=successUpdate');
        }else{
            header('Location: jadwalMengajar-edit?idMengajar='.$id.'&act=failUpdate');
        }
    }else{
        header('Location: jadwalMengajar-edit?idMengajar='.$id.'&act=emtpy');
    }
}
?>
            ...
            <div class="w-full h-[4rem] m-6 p-4 rounded-lg bg-slate-800 text-zinc-300">
                ...
                <div class="py-3">
                    ...
                    ?>
                        ...
                        </svg><span class='font-bold'>&nbspSUCCESS MELAKUKAN UPDATE JADWAL MENGAJAR &nbspAnda Akan Diarahkan ke JADWAL MENGAJAR&nbsp</span>
                    ...
                    <?php
                        ...
                        <span class='font-bold'>&nbspGAGAL MELAKUKAN UPDATE JADWAL MENGAJAR , DATA TIDAK BOLEH KOSONG</span> 
                        </div>
                    <?php
                    }else{
                         ?>  
                        ...
                        <span class='font-bold'>&nbspGAGAL MELAKUKAN UPDATE JADWAL MENGAJAR , SILAHKAN PERIKSA DATA ANDA KEMBALI</span> 
                        </div>
                    <?php
                    }
                }
                    ?>
                    <?php
                        while($row = mysqli_fetch_array($editDataMengajar)){
                    ?>
                        <h2 class="font-bold text-2xl sm:text-xl"><button class="px-3 py-1 bg-indigo-500 hover:bg-slate-900" onclick="window.location.href='RekapAbsensi'"><</button>&nbspFORM EDIT JADWAL MENGAJAR</h2>

                        <div class="relative">
                            <label for="">MATA PELAJARAN</label>
                            <input type="text" name="mapel" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan NAMA PELAJARAN"value="<?= $row['mata_pelajaran'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">GURU PENGAJAR</label>
                            <input type="text" name="guru_pengajar" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan GURU PENGAJAR" value="<?= $row['pengajar'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">KELAS MENGAJAR</label>
                            <input type="text" name="kelas_mengajar" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan KELAS MENGAJAR" value="<?= $row['kelas_mengajar'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">LINK VIRTUAL</label>
                            <input type="text" name="link_virtual" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan Link Virtual" value="<?= $row['link_virtual'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">PASSWORD LINK VIRTUAL</label>
                            <input type="text" name="jam_mulai" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan JAM MULAI" value="<?= $row['pass_link_virtual'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">JAM MULAI</label>
                            <input type="text" name="jam_mulai" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan JAM MULAI" value="<?= $row['jam_mulai'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">JAM AKHIR</label>
                            <input type="text" name="jam_akhir" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan JAM AKHIR" value="<?= $row['jam_akhir'] ?>">
                        </div>
                        <div class="relative">
                            <label for="">STATUS</label>
                            <input type="text" name="status_pelajaran" class="absolute flex items-center p-2 w-full bg-transparent border border-gray-600 text-left text-zinc-400 focus:outline-none focus:border-indigo-500 transform translate duration-500" placeholder="Masukkan STATUS" value="<?= $row['status'] ?>">
                        </div>
                        
                        <div class=""></div>
                            <button type="submit" name="simpan" class="bg-purple-dark text-purple-600 p-2 hover:bg-purple-500 hover:text-white hover:transform duration-300">Simpan Data</button>
                    </form>
                    <?php
                        }
                    ?>
