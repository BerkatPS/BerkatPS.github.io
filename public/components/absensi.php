...
if(isset($_POST['submitabsen'])){
    ...
    $tanggal = date('d F Y');
    $waktu = date('H:i:s');
    if($time['tm_hour'] >= 5 OR $time['tm_hour'] >= 7 AND $time['tm_min'] >= 30){
        $checkNis = $confg->query("SELECT * FROM user WHERE NIS = $_SESSION[nis]");
        $countNis = mysqli_num_rows($checkNis);
        if($countNis > 0){
            $cekAbsensiTerbaru = $confg->query("SELECT * FROM tbl_absensi_siswa WHERE id_absensi_siswa = $_SESSION[userId] AND tanggal = '$tanggal'");
            $countAbsensiTerbaru = mysqli_num_rows($cekAbsensiTerbaru);
            if($countAbsensiTerbaru === 0){
                if($time['tm_hour'] >= 5 OR $time['tm_hour'] >= 7){
                    $insertHadir = $confg->query("INSERT INTO tbl_absensi_siswa(id_absensi_siswa,tanggal,waktu,nis,nama,kelas,status,poin) VALUES('$_SESSION[userId]','$tanggal','$waktu','$_SESSION[nis]','$_SESSION[user]','$_SESSION[kelas]','HADIR',0)");
                    header('Location: absensi?act=successAbsen');
                }
                else if($time['tm_hour'] > 7 AND $time['tm_min'] < 30){
                    $insertTerlambat = $confg->query("INSERT INTO tbl_absensi_siswa(id_absensi_siswa,tanggal,waktu,nis,nama,kelas,status,poin) VALUES('$_SESSION[userId]','$tanggal','$waktu','$_SESSION[nis]','$_SESSION[user]','$_SESSION[kelas]','TERLAMBAT',5)");
                    $updatePoin = $confg->query("UPDATE user SET poin=poin+5 WHERE id = $_SESSION[userId]");
                    header('Location: absensi?act=successAbsen');
                }else{
                    header('Location: absensi?act=failAbsen');
                }
            }else{
                header('Location: absensi?act=forbiddensubmit');
            }
        }else{
            header('Location: absensi?act=notFoundAccount');
        }
    }else{
        header('Location: absensi?act=closed');
    }
...
$countIzin = $confg->query("SELECT COUNT(id_absensi_siswa) AS id_absensi4 FROM tbl_absensi_siswa WHERE status='IZIN' OR status='SAKIT' AND id_absensi_siswa = $id");
$field = mysqli_fetch_assoc($countIzin);
...
// if($time['tm_hour'] > 8 && !$insert){
//     $insert = $confg->query("INSERT INTO tbl_absensi_siswa(id_absensi_siswa,tanggal,Hari,nis,nama,kelas,status,poin) VALUES($id,'$tanggal','$waktu','$nama','$kelas',$NIS,'ALPHA',10)");
// }
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <div class="container flex justify-end items-center mx-auto">
                ...
                <div class="relative overflow-x-auto bg-slate-800 shadow-md rounded-lg text-gray-400 pt-5">
                    ...
                    <div class="relative px-4 pb-5">
                        <h1 class="font-bold text-lg">TABLE ABSENSI (&nbsp<?= strtoupper($_SESSION['user']) ?>&nbsp)</h1>
                    ...
                    <div class="px-4 pb-5 gap-5 grid md:grid-cols-4 sm:grid-cols-2 font-bold ">
                        <div class="w-full bg-red-600 grid grid-cols-1 sm:grid-cols-2 text-white rounded-lg">
                            <div class="block">
                                ...
                                <h2 class="px-4"><?=  $field4['id_absensi3']?></h2>
                            </div>
                        ...
                        <div class="w-full bg-blue-dark grid grid-cols-1 sm:grid-cols-2 text-sky-600 rounded-lg">
                             <div class="block">
                                ...
                                <h2 class="px-4"><?= $field['id_absensi4'] ?></h2>
                            </div>
                    ...
                    <table class="w-full text-sm text-center text-gray-400 dark:text-gray-400 rounded-lg" id="example">
                        ...
                        ?>
                            <tr class="text-center relative ">
                                ...
                                <td class="px-6 py-4 bg-slate-800 font-medium"><?= $row['waktu'] ?></td>
                                ...
                                <?php
                                    ...
                                    echo'<span class="bg-red-700 text-white text-center p-2">'.$row['status'].'</span>';
                                    }else{
